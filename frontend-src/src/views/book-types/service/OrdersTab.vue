<template>
  <main class="d-flex flex-1 overflow-hidden mobile:flex-col relative">
    <aside class="col-6 d-flex flex-col overflow-hidden border-right mobile:max-w-full flex-grow mobile:w-full">
      ...
      <div v-if="feedbackMessage" class="alert alert-success m-4" role="status">
        {{ feedbackMessage }}
      </div>
    ...
    <aside class="col-6 mobile:max-w-full flex-grow mobile:w-full">
      <section class="flex-1 overflow-y-auto p-4 bg-lower h-full">
          ...
          <div class="card-body">
            <header class="d-flex mb-2">
              <div class="flex-grow">
                ...
                <p class="text-secondary text-xs">{{ $t('service.orders.orderId') }}: {{ selectedOrder.id }}</p>
              </div>
              <div>
            ...
            <div class="table-responsive">
              <table class="table table-bordered">
                <thead>
                  <tr>
                    <th width="28%">{{ $t('service.fields.objectName') }}</th>
                    <th width="22%">{{ $t('service.fields.serviceType') }}</th>
                ...
                <tbody>
                  <tr v-for="item in selectedOrderItems" :key="item.id">
                    <td>
                      ...
                      <small v-if="item.note" class="text-secondary">{{ item.note }}</small>
                    </td>
        ...
        <p v-else class="text-lg p-10 text-secondary text-center">
          {{ $t('service.orders.selectOrderPlaceholder') }}
        </p>

        <div class="card card-body mt-5">
          <div class="d-flex justify-content-between align-items-center gap-2 mb-4">
            <h6 class="text-lg mb-0">{{ $t('service.serviceTypes.title') }}</h6>
            <button type="button" class="btn btn-neutral" @click="openCreateServiceTypeDialog">
            ...
            <table class="table" id="book_service_type">
              ...
              <tbody>
                <tr v-for="serviceType in serviceTypes" :key="serviceType.id">
                  ...
                  <td class="text-right">
                    <button type="button" class="btn btn-link p-0 mr-3" @click="openEditServiceTypeDialog(serviceType)">
